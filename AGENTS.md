...
**Docs map** (`docs/` is the documentation root; planning/dev are local-only, while harness contracts in `docs/agents/` are committed):
...
- `docs/agents/` — portable issue-tracker and domain-documentation contracts. Read `issue-tracker.md` for issue or Wayfinder work and `domain.md` before creating context glossaries or ADRs.
...
# Mutation testing (on demand only — slow, never part of verify.mjs or CI)
dotnet stryker
```
...
- Never report work as done from your own assessment. Done = `node scripts/verify.mjs` exits 0.
...
Run `node scripts/verify.mjs` — it is the final vote: locked restores, backend build, format/lint gates, tests, frontend build, secret scanning, and workflow security in one deterministic command.
